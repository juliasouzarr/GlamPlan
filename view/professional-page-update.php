...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cpf = $_POST['cpf'] ?? '';
    $password = !empty($_POST['password']) ? $_POST['password'] : $professional['password'];

    if (empty($name) || empty($user) || empty($cpf)) {
        die("Nome, usuário e CPF são obrigatórios.");
    }
...
<body>
    <div id="container-login">
        ...
        <form action="professional-page-update.php?id=<?= $professional['id']; ?>" method="POST">
            <input type="text" placeholder="Nome" class="login-input" name="name" value="<?= htmlspecialchars($professional['name']); ?>" required>
